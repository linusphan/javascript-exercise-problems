/*
input 
  - string of email data

output
  - two basic statistics about the string
    - number of email messages found in the string
    - the date range of the email messages

rules
  - email messages string
    - contains multiple email messages separated by the delimiter string ##||##
    - each email message has 5 parts. The delimiter string #/# separates the parts
      - The five parts
        ...
        - recipient
        - body
      - parts occur in the sequence shown above
*/
...
function mailCount(emailData) {
  var emails = emailData.split('#||#').map(x => x.split('#/#'));

  var sortedDates = emails.map(x => x[2])
    .map(x => x.split(' '))
    .map(x => x[1])
    .map(x => x.split('-'))
    .map(x => x.map(y=>Number(y)))
    .map(x => new Date(x[2], x[0] - 1, x[1]))
    .sort((a,b) => a.getTime() - b.getTime());

  var dates = [sortedDates[0], sortedDates[sortedDates.length - 1]];
  dates = dates.map(x => String(x).split(' ').slice(0, 4).join(' '));
...
function getDateObjects(dates) {
  return dates.map(function (dates) {
    var dateElements = dates.split(' ')[1].split('-');
