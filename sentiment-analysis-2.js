...
function message(positives, negatives) {
  var positivesCount = positives.length;
  var negativesCount = negatives.length;

  var result;

  if (positivesCount - negativesCount > 0) {
    result = 'Positive';
  } else if (positivesCount - negativesCount < 0) {
    result = 'Negative';
  } else {
    result = 'Neutral';
  }

  console.log(`There are ${positivesCount} positive words in the text.
...
function sentiment(text) {
  ...
  while (matchedArray = positiveRegex.exec(text)) {
    positives.push(matchedArray[0]);
  }

  while (matchedArray = negativeRegex.exec(text)) {
    negatives.push(matchedArray[0]);
  }

  console.log(message(positives, negatives));
}
